...
class Ctrl extends UIController {
  data = signal({name: 'a'})
  val = 1;
  override dispose(): void {

  }
  override updateProps(p): void {
    const aPb = this.calc(p.v1, p.v1);
    this.calcThat(aPb, p.v2);
  }

  calcThat = this.memo((a, b) => {
    return a + b;
  }

  calc = this.memo((a, b, r){
    // data.value = {name: a + b}

    const dispose = connectToResource(a, r)
...
function Comp(props: {}){
  const c = useLocalController(Ctrl, props);
  


  return <div>{c.data.name}</div>
}
