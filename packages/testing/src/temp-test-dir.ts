import type { ITempDirectory } from 'create-temp-directory';
 ...
 */
export async function createTestDir(prefix?: string | undefined, disposalGroup = DISPOSE_OF_TEMP_DIRS) {
    if (typeof globalThis.window === 'undefined') {
        const createTempDirectory = (await import('create-temp-directory')).createTempDirectory as (
            prefix?: string
        ) => Promise<ITempDirectory>;
        const dir = await createTempDirectory(prefix);
        // eslint-disable-next-line @typescript-eslint/unbound-method
        disposeAfter(dir.remove, disposalGroup);
        return dir.path;
    } else {
        throw new Error('createTestDir is not supported in browser');
    }
}
