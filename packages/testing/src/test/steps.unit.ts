...
describe('withSteps', () => {
    ...
    describe('promise step', () => {
        const LONG_TIME = 10;
        const SHORT_TIME = 1;
        it('times out with the description', withSteps(async(step) => {
            await expect(
                step.withTimeout(sleep(LONG_TIME)).timeout(SHORT_TIME).description('test')
            ).to.eventually.rejectedWith('test');
        }));
        it('fulfils the promise in the allotted time', withSteps(async(step) => {
            expect(await step.withTimeout(sleep(SHORT_TIME).then(() => 'success')).timeout(LONG_TIME)).to.equal('success');
        }));
    });
    ...
    describe('firstCall', () => {
        ...
        it('resolves with the call arguments', withSteps(async(step) => {
            const call = step.waitForCall(target, 'method');
            target.method(1, 'success');
            expect(await call).to.eql([1, 'success']);
        }));
        it('times out if not called', withSteps(async(step) => {
            await expect(step.waitForCall(target, 'method').timeout(1).description('timeout')).to.eventually.rejectedWith(
                'timeout'
            );
        }));
        it('calls thru to the original method', withSteps(async(step) => {
            const call = step.waitForCall(target, 'method');
            target.method(1, 'success');
            await call;
            expect(target).to.deep.contain({ a: 1, b: 'success' });
        }));
        it('restores the original method after the step is done', withSteps(async(step) => {
    ...
    describe('waitForStubCall', () => {
        it('resolves to {callArgs, returned}', withSteps(async (steps) => {
            expect(
        ...
        it('times out when the stub is not called', withSteps(async(steps) => {
            await expect(
                steps
